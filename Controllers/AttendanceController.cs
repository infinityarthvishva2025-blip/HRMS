    ...
    ICompOffService compOff,
     ...
     "Geo Check-in Present"
        ...
        public IActionResult Index(string search, DateTime? fromDate, DateTime? toDate, string status)
        {
            ...
            // =========================================
            // BASE QUERY (JOIN Attendance + Employees)
            // =========================================
            var attendance =
            ...
            attendance = attendance
                .Where(x => x.a.Date >= start && x.a.Date <= end);
            ...
            if (!string.IsNullOrWhiteSpace(search))
            {
                ...
                attendance = attendance.Where(x =>
                    x.a.Emp_Code.Contains(search) ||
                    x.e.Name.Contains(search));
            }

            // ---------------------------------------------
            // 3️⃣ STATUS FILTER (FINAL CORRECT VERSION)
            // ---------------------------------------------

            if (string.IsNullOrEmpty(status))
            {
                // First page load → only Active employees
                attendance = attendance
                    .Where(x => x.e.Status == "Active");
            }
            ...
            {
                attendance = attendance
                    .Where(x => x.e.Status == "Inactive");
            }
            else if (status == "All")
            {
                // Show ALL employees (do nothing)
            }
            else if (status == "NotCheckedOut")
            {
                attendance = attendance
                    .Where(x => x.a.InTime != null && x.a.OutTime == null);
            }
            ...
            var list = attendance
                ...
                {
                    Emp_Code = x.a.Emp_Code,
                    AttDate = x.a.Date,
                    InTime = x.a.InTime,
                    OutTime = x.a.OutTime,
                    Status = x.a.Status,
                    TotalHours = (x.a.InTime != null && x.a.OutTime != null)
                        ? (x.a.OutTime.Value - x.a.InTime.Value).ToString(@"hh\:mm")
                        : "--",

                    CorrectionRequested = x.a.CorrectionRequested,
                    CorrectionStatus = x.a.CorrectionStatus,

                    CheckInLatitude = x.a.CheckInLatitude,
                    CheckInLongitude = x.a.CheckInLongitude,
                    CheckOutLatitude = x.a.CheckOutLatitude,
                    CheckOutLongitude = x.a.CheckOutLongitude,
                })
                .ToList();
        ...
        public IActionResult ExportFiltered(string search, DateTime? fromDate, DateTime? toDate, string status)
        {
            ...
            var employees = _context.Employees
                ...
                .ToDictionary(e => e.EmployeeCode, e => e.Name);
            ...
            using (var pkg = new ExcelPackage())
            {
                ...
                foreach (var a in list)
                {
                    ...
                    if (!employees.ContainsKey(a.Emp_Code))
                        continue;

                    ws.Cells[row, 1].Value = a.Emp_Code;
                    ws.Cells[row, 2].Value = employees[a.Emp_Code];
