    ...
    public class LeaveController : Controller
    {
        ...
        public async Task<IActionResult> Create()
        {
            ...
            ViewBag.CompOffBalance =
                await _compOff.GetActiveBalanceAsync(empId.Value);

            ViewBag.CompOffDates = await _context.CompOffLedgers
                .Where(x => x.EmployeeId == empId && x.Status == "Active")
        ...
        // ===================== VALIDATE COMPOFF =====================
        private async Task<bool> ValidateCompOffAsync(int employeeId, double days)
        {
            await _compOff.ExpireOldCompOffAsync(DateTime.Today);
        ...
        // ===================== PROCESS APPROVAL =====================
        private async Task ProcessApproval(int id, string role, bool approve)
        {
            ...
            switch (role)
            {
                ...
                case "GM": leave.ManagerStatus = approve ? "Approved" : "Rejected"; break;
            ...
            leave.NextApproverRole =
                await _workflow.GetNextApproverRoleAsync(role);

            // ✅ FINAL APPROVAL
            if (leave.NextApproverRole == "Completed")
            {
                if(leave.CurrentApproverRole=="Director" || leave.CurrentApproverRole=="VP")
                    leave.OverallStatus = "Approved";

                // 🔥 USE COMPOFF
                if (leave.LeaveType == "coff")
                {
                    bool ok = await _compOff.ReserveAndUseCompOffAsync(
                        emp.Id, (int)leave.TotalDays, leave.Id);
        ...
        private async Task SyncLeaveToAttendance(Leave leave)
        {
            ...
            for (var d = leave.StartDate; d <= leave.EndDate; d = d.AddDays(1))
                await MarkLeave(emp.EmployeeCode, d, leave.LeaveType);
        }

        private async Task MarkLeave(string empCode, DateTime date, string type)
        {
            var row = await _context.Attendances
                .FirstOrDefaultAsync(a => a.Emp_Code == empCode && a.Date == date);

            var status = type == "coff" ? "Coff" : "L";
        ...
        private async Task RevertLeaveInAttendance(Leave leave)
        {
            ...
            foreach (var r in rows)
                r.Status = r.InTime != null && r.OutTime != null ? "P" : "A";
        }

        // ===================== APPROVAL ENDPOINTS =====================
        public async Task<IActionResult> HrApprove(int id, bool approve)
        {
            await ProcessApproval(id, "HR", approve);
            return RedirectToAction("HrApprovalList");
        }

        public async Task<IActionResult> ManagerApprove(int id, bool approve)
        {
            await ProcessApproval(id, "GM", approve);
            return RedirectToAction("ManagerApprovalList");
        }

        public async Task<IActionResult> VpApprove(int id, bool approve)
        {
            await ProcessApproval(id, "VP", approve);
            return RedirectToAction("VpApprovalList");
        }

        public async Task<IActionResult> DirectorApprove(int id, bool approve)
        {
            await ProcessApproval(id, "Director", approve);
        ...
        public async Task<IActionResult> CompOffHistory()
        {
            ...
            return View(await _compOff.GetHistoryAsync(empId.Value));
        }
        // -------------------- GET MY LEAVES --------------------
        ...
        public async Task<IActionResult> LeaveReport()
        {
            ...
            var pendingVP = await _context.Leaves.CountAsync(l =>
               l.HrStatus == "Approved" &&
               l.ManagerStatus == "Approved" &&
               l.VpStatus == "Pending");
...
//                await MarkLeave(empCode, d, leave.LeaveType);

//            // Sandwich Rule
//            if (start.AddDays(-1).DayOfWeek == DayOfWeek.Sunday)
//                await MarkLeave(empCode, start.AddDays(-1), leave.LeaveType);

//            if (end.AddDays(1).DayOfWeek == DayOfWeek.Sunday)
//                await MarkLeave(empCode, end.AddDays(1), leave.LeaveType);

//            await _context.SaveChangesAsync();
//        }

//        private async Task MarkLeave(string empCode, DateTime date, string leaveType)
//        {
//            var row = await _context.Attendances
//                .FirstOrDefaultAsync(a => a.Emp_Code == empCode && a.Date == date);

//            // decide status BEFORE object creation
//            var status = leaveType == "coff" ? "Coff" : "L";
